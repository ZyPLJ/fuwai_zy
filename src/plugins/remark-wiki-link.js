 ...
 * 适配自 Firefly (CuteLeaf/Firefly) src/plugins/remark-wiki-link.js
 * 差异：本项目 posts 无 frontmatter slug 字段（URL 恒为 /posts/<dir>/<filename>/），
 *       无 api 封面分支；entry.id 由 Astro glob loader 直接取文件路径，不做 index 剥离。
 */
...
/**
 * 由文章文件的绝对路径反推 content path（即 Astro 的 entry.slug）。
 */
...
function createCoverNode(meta, context) {
	const image =
...
function createWikiLinkCard(parsed, context) {
	...
	const resolvedPath = toContentPath(meta.filePath);
	const title =
		resolveAlias(parsed, meta) ||
		(typeof meta.data.title === "string" && meta.data.title
			? meta.data.title
			: resolvedPath);
	// password 加密文章不展示摘要（与 Firefly 一致）
	const encrypted =
	...
	const cover = createCoverNode(meta, context);
	if (cover) {
...
function createWikiLink(value) {
	...
	const pageUrl = parsed.contentPath
		? createPostUrl(meta ? toContentPath(meta.filePath) : parsed.contentPath)
		: "";
 ...
 *
 * Targets resolve in two steps: exact file path (`<dir>/<filename>` or
 * `<dir>/<filename>.md`), then bare file name (for Obsidian's "shortest path
 * when possible" format, accepted only when unique). URLs are always derived
 * from the resolved post's file path rather than from the link text.
 */
